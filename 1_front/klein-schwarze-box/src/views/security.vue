        ...
        <li>
          ...
          <div v-show="step === 1">
            <div class="description">
              ...
              <p>
                Hash 함수는 한 입력에 대해 n자리의 수학적으로 거의 유일한 값을 출력하는 함수이다. 
                출력된 값으로 본래 입력 값을 유추할 수 없다는 제2 역상 저항성과 유일한 값, 즉 다른 값일 때 그 결과가 모두 다르다는 점에서 충돌 저항성이라는 특징이 있다.
                Hash 함수를 이용하여 암호화 등에 활발하게 사용되고 있기 때문에 보안 분야의 핵심 개념 중 하나이다. 
                더불어 이러한 Hash 함수는 블록체인의 논리에 가장 기초적인 요소이며 블록들의 체인 형성에 사용되어 데이터 무결성 확보에 핵심이 된다.
              </p>
            </div>
            <div class="description">
              <h4>Instruction</h4>
              <p>
                Hash 함수를 직접 실행하여 개념을 파악한다.<br><br>
                [기본 조작]<br>
                1. Data에 텍스트를 입력한다.<br>
                2. 오른편 'Generate Hash' 버튼을 클릭한다.<br>
                3. Hash 부분에 결과값(Digested)를 확인한다.<br>
                <br>
                [역상 저항성 테스트]<br>
                역상 저항성이란 그 결과값으로 입력 값을 역산할 수 없다는 특징이다.<br><br>
                1. Hash된 결과를 Data에 입력한다.<br>
                2. 오른편 'Generate Hash' 버튼을 클릭한다.<br>
                3. 결과를 확인한다.<br>
                <br>
                [충돌 저항성 테스트]<br>
                충돌 저항성이란 한 입력에 대해서 그 결과가 유일하다는 특징이다.<br>
                즉 어떠한 입력을 넣어 Hash 하더라도 그 결과는 모두 다르다. 즉 충돌되지 않는다.<br><br>
                1. Data에 텍스트를 입력한다. <br>
                2. 오른편 'Generate Hash' 버튼을 클릭한다.<br>
                3. 결과를 확인하고 메모한다.<br>
                4. 1~2를 반복하며 3의 결과와 동일한 결과가 나오는지 확인한다.<br>
              </p>
            </div>
          </div>
        </li>
